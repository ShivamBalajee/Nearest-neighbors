...
#
# Submitted by: Parth Verma -- paverma
#
...
class KNearestNeighbors:
    ...
    def predict(self, X):
        ...
        ans = []
        for test_data in X:
            a = map(lambda x: (x[1], self._distance(x[0], test_data)), zip(self._X, self._y))
            a = sorted(a, key=lambda x: x[1])[:self.n_neighbors]
            if self.weights == 'uniform':
                a = Counter(map(lambda x: x[0], a))
            else:
                counts = defaultdict(float)
                for label, score in a:
                    counts[label] += 1 / (score + 0.0000001)
                a = counts
            ans.append(max(a.items(), key=lambda x: x[1])[0])
        return np.array(ans)
